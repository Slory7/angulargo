...
func (c *UserController) PutMemo(ctx iris.Context) (dto v.UserMemoDto, err error) {
	err = ctx.ReadJSON(&dto)
	if err == nil {
		if err = globals.Validator.Struct(dto); err == nil {
			poco := m.UserWithMemo{}
			copier.Copy(&poco, &dto)
			affect, err := c.Service.UpdateUserMemo(&poco)
			if err != nil {
				return dto, err
			}
			if affect != 1 {
